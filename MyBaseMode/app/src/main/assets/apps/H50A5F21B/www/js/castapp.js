...
window.dongyi = window.dj = window.castapp = window.ca =  {
	...
	checkRendered:function(sub){
		sub.checkRenderedContent({}, function(e){
			if(e.rendered){
				dongyi.closeStartPage();
			}else{
				dongyi.checkRendered(sub);
			}
		}, function(e){
				dongyi.checkRendered(sub);
		});	
...
//				console.log(json.url+'?'+json2url(json.data));
	...
	dateSelect:function(jsonData){
		...
		gM.plusReady(function(){
			plus.nativeUI.pickDate( function(e) {
				...
				jsonData.callback && jsonData.callback(d.getFullYear()+"-"+(Month)+"-"+ day);
		    },function(e){        
		        jsonData.callback && jsonData.callback(false);
				        	...
				        	dongyi.camera({
				        		succFn:function(path,name){
					        		if(jsonData && jsonData.isUpload){
										...
										function(i) {
											dongyi.uploadFiles(jsonData.uploadUrl,i.target,function(imgPath){
	...
	showWaiting:function(watingPrompt){
		gM.plusReady(function(){
			gP.nativeUI.showWaiting(watingPrompt,{modal:false,back:close});
		})
	},
	/**
	 * 关闭等待框
	...
	callPhone:function(targetPhone){
		gP.device.dial(targetPhone,true);
	},
	/**	
	 * 邮件
	...
	uploadFiles:function(uploadUrl,filePath,succFn,errFn){
		...
		files.push({name:"upfile",path:filePath});       
		if(files.length<=0){
	    ...
	    for(var i=0;i<files.length;i++){
	        var f=files[i];
	        task.addFile(f.path,{key:'upfile'});
	    }
	    task.start();
	},
	/**
...
// 			}
			...
			mui('#middlePopover0').on('tap','.popover-div2-div',function(){
...
// 			);	
	...
	combinationSelect:function(selectNumber,data,callback){
		...
		selector.show(function(items) {
			...
			if(selectNumber == 1){
				...
				backArr = [{te:items[0].text+'/'+items[1].text,va:items[1].value}];
	...
	combinationSelect1:function(selectNumber,data,callback){
		if(!selectNumber){selectNumber = 1}; 
		var selector = new gM.PopPicker({
			layer: selectNumber
		});
	...
	checkTheNetwork:function(){
		ca.getMachineInfo(function(data){
			...
			if(data.network=='2G蜂窝网络'){
				plus.nativeUI.closeWaiting();
				mui.toast('网络环境较差,稍后再试！');
				return;
			}
	 ...
	 * 设置状态栏颜色为白色
	 */
	whiteStatusBar:function(){
		mui.plusReady(function(){
			plus.navigator.setStatusBarStyle('light');//light字体是白色
		})
	},
	/*
	 * 设置状态栏颜色为黑
	 */
	blackStatusBar:function(){
		mui.plusReady(function(){
			plus.navigator.setStatusBarStyle('dark');//dark
		})
	},
	
	checkClientid:function(urls,ids){
		var url=localStorage.getItem('url');
		var userid=localStorage.getItem('userid');
		var clientid=localStorage.getItem('clientid');
		gM.get(url+'findClientid',{user_id:userid},function(data){
				if(clientid!=data.clientid){
					if(localStorage.getItem('userzhuangtai')==1){
						
					}else{
						var btnArray = ['是','否'];
						gM.confirm('当前账号在别的手机登录，可能会导致您接收不到消息，请您重新登录。', '提示!', btnArray, function(e) {
		                    if (e.index == 0) {
		                    	localStorage.removeItem('userid');
		                    	localStorage.removeItem('e_id');
		                    	localStorage.removeItem('structure_id');
		                    	localStorage.removeItem('isadmin');
		                    	localStorage.removeItem('userzhuangtai');
		                    	ca.newInterface({url:urls,id:ids})
		                    } else {
		                    	localStorage.setItem('userzhuangtai',1);
		                    }
		                })
					}
	...
	ajaxs:function(json){
		...
		mui.ajax(localStorage.getItem('url')+json.url,{
			data:json.data,
			dataType:'json',//服务器返回json格式数据
			type:'post',//HTTP请求类型
			timeout:10000,//超时时间设置为10秒；
			headers:{'Content-Type':'application/json','userid':localStorage.getItem('userid'),'password':localStorage.getItem('password')},	              
			success:function(data){
				//服务器返回响应，根据响应结果，分析是否登录成功；
				json.succFn && json.succFn(data);	
			},
			error:function(xhr,type,errorThrown){
				plus.nativeUI.closeWaiting();
				var call="请求错误";
				//异常处理；
				if(type=='timeout')call='请求超时';
				if(type=='abort')call='网络连接中断';
				if(type=='null' || type=='error')call='服务器错误';
				json.errFn && json.errFn(call);
			}
		});
	},
	/**
	 ...
	 * @param {Object} num
	 */
	localStorageUser:function(num){
		var user=localStorage.getItem('account_switch');
		var arr=JSON.parse(user);
		var users=arr[num];
		if(!ca.emptyFun(users)){
			if(new Date().getTime()-users.loginlasttime<86400000*15){
				users.loginlasttime=new Date().getTime();
				arr.splice(num,1,users);
				...
				localStorage.setItem('isadmin',users.isadmin);
				localStorage.setItem('username',users.username);
				localStorage.setItem('password',users.password);
				localStorage.setItem('province',users.province);
				localStorage.setItem('city',users.city);
				localStorage.setItem('e_id',users.e_id);
				localStorage.setItem('im_token',users.im_token);
				localStorage.setItem('structure_id',users.structure_id);
				localStorage.setItem('e_name',users.e_name);
				localStorage.setItem('structure_name',users.structure_name);
				localStorage.setItem('e_code',users.e_code);
				localStorage.setItem('realname',users.realname);
				localStorage.setItem('userimg_url',users.userimg_url);
				return true;
			}else{
				arr.splice(num,1);
				localStorage.setItem('account_switch',JSON.stringify(arr));
	 ...
	 * @param {Object} id
	 */
	deleteLocalStorageUser:function(id){
		var user=localStorage.getItem('account_switch');
		var arr=JSON.parse(user);
		for(var a in arr){
			if(arr[a].userid==id){
	...
	/**
	 * 对数组排序
	 * @param {Object} order 方式
	 * @param {Object} sortBy 排序字段
	 */
	...
	createHistory:function(data){
		...
		if(!ca.emptyFun(historylist)){
			for(var a in historylist){
				if(historylist[a].type==data.type && historylist[a].id==data.id){
					historylist[a].date=data.date;
					plus.storage.setItem(account+'history',JSON.stringify(historylist));
					break;
				}else if(a==historylist.length-1){
					var ob={
						id:data.id,
						name:data.name,
						type:data.type,
						comment_num:data.data_comment_num,
						data_grade:data.data_grade,
	...
	thisTimeForm:function(hour, time) {
		...
		if(year == thisyear && month == thismonth && day == thisday) {
			...
			if(years == 0 && months == 0 && days > 0 && days < 7) {
				switch(days) {
					...
					default:
						return "星期" + "日一二三四五六".charAt(time.getDay())+' '+hour+':'+minutes;
						break;
				}
			} else {
	...
	// 下载文件
	downloader: function(data) {
		var dtask = plus.downloader.createDownload(data.url, {
			filename:'_downloads/'+data.filePath
